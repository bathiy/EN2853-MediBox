...
void connectToBroker(){
  while(!mqttClient.connected()){
    ...
    if(mqttClient.connect("ESP32-5255")){
      Serial.println("Connected");
      //subscribes
      mqttClient.subscribe("ts");
      mqttClient.subscribe("tu");
      mqttClient.subscribe("Theta_off");
      mqttClient.subscribe("gama");
...
void receiveCallback(char* topic, byte* message, unsigned int length){
  ...
  if (String(topic) == "ts" && payload.toInt() >= 1000) {
    Serial.println(topic);
    ts = payload.toInt();
    Serial.printf("Updated: ts = %lu ms\n", ts);
  } 
  if (String(topic) == "tu") {
    Serial.println(topic);
    tu = payload.toInt();
    Serial.printf("Updated: tu = %lu ms\n", tu);
  }
  if (String(topic) == "Theta_off") {
    Serial.println(topic);
    theta_offset = payload.toInt();
    Serial.printf("Updated: Theta off = %lu \n", theta_offset);
  }
  if (String(topic) == "gama") {
    Serial.println(topic);
    gama = payload.toFloat();
    Serial.printf("Updated: Gmma = %.2f \n", gama);
  }
  if (String(topic) == "T_med") {
    Serial.println(topic);
    Tmed = payload.toFloat();
    Serial.printf("Updated: Tmed = %.2f \n", Tmed);
  }
} 
...
//Light intensity average taking and publish
void getAverageLightIntensity() {
  unsigned long now = millis();
  //Get the sum of the intensity value each ts time
  if (now - lastSampleTime >= ts) {
    lastSampleTime = now;
  ...
  // Publish average every tu milliseconds
  if (now - lastAverageTime >= tu && count > 0) {
    lastAverageTime = now;

    average = sum / count;

    char buffer[16];
    dtostrf(average, 1, 4, buffer);  

    mqttClient.publish("Average_LDR", buffer); 
    Serial.print("Published average: ");
...
void shadeControl(){
  ...
  char temp[16];
  dtostrf(data.temperature, 1, 4, temp);

  mqttClient.publish("Temperature", temp); 

  float T = data.temperature;
  int ts_s = ts / 1000; //ts in seconds
  int tu_s = tu / 1000; //tu in seconds
  float ln_ts_tu = log((float)ts_s / (float)tu_s);  
  float theta = theta_offset + (180.0 - theta_offset) * average * gama * ln_ts_tu * (T / Tmed);

  theta = constrain(theta, theta_offset, 180.0); 
  int thetaPass = (int)theta;

  myServo.write(thetaPass); 
}